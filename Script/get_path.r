...
get_fig_fname_start <- function( model = NULL, m = NULL, celltype, model_id, mcmc_pars )
{
  if ( is.null( m ) ) { m <- model }
  if ( ! is.null( m ) ) {
    start <- sprintf( 
      "%s_%s_%s", m$celltype, m$model_id, m$mcmc_pars$id )
  } else {
...
get_model_id <- function( file_name, from_file = "sel_models", model = NULL, m = NULL )
{
  if ( is.null( m ) ) { m <- model }
...
get_max_chain <- function( m )
{
  ...
  if ( file.exists( sel_chains_csv ) ) {
    ...
    max_chain <- 
      sel_chains_table$sel_chains_set[ 
        sel_chains_table$sel_chains_type == "max" ]
    # older version:
    if ( is.null( max_chain ) ) {
      max_chain <- 
        sel_chains_table$sel.chains_set[ 
