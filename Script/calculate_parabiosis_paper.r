...

{
  ...
  if ( ! exists( "CALC_TTP" ) ) { CALC_TTP <- TRUE }
  ...
  # only one tissue when run from Shell script
  ...
  # Not to do in Tissue Treg paper 
  # but do in  Parabiosis modelling paper
  if ( ! CALC_TTP ) {
    for ( TIS_I in TISSUES ) {
      # Figure only for a specific tissue at the end here - it takes extra time
      sim_10000 <- sim_combined_dwell( m = m, n_sim = 10000 )
      plot_combined_dwell( m = m, sim = sim_10000, export = TRUE )
      plot_16_comb_dwells( m_tisNA = get_m_tisNA( m = m ) )
    }  
    plot_exit_types( m_tisNA = m_tisNA )
    get_rel_flows_and_exits( m_tisNA = m_tisNA )
  }
  
  if ( CALC_TTP ) { sessionInfo() }
