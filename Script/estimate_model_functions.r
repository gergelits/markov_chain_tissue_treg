...
{       
  ...
  if ( GET_FITTED_PARAMETERS_FROM_MCMC_SAMPLE_TO_R_ENV <- TRUE ) {
    ...
    if ( GET_ALL_THE_CALCULATED_PARAMS_TO_THE_ENV <- TRUE ) {
      ...
      # Assign the modes of !!VECTORED & SAMPLED!! values from .stan model to R variables
    ...
    { 
      ...
      for ( pop in pb.popul )
      {
        ...
        ggsave( sprintf( "%s/%s_%s.png", chain_dir, tis, pop ), ggfig, 
                width = 8, height = 6 )
        l_ggfigs[[ which( pb.tissue == tis ) ]][[ which( pb.popul == pop ) ]] <- 
          ggfig
      }
    }
