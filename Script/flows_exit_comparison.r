...
celltype <- "Treg"
source( "testing_model_setup.r", local = TRUE )
}
...
get_rel_flows_and_exits <- function( m_tisNA = m_tisNA )
{
  export <- TRUE
  tissues <- get_pmc()$dTissueAllOrderedGroup$tissue.all.ordered[ 1 : 17 ]
  ...
  d_exits_all <- get_exits_all( m_tisNA = m_tisNA )
  
  rel_flow_tissues_all %>% 
    ...
    dplyr::select( 
      celltype, tissue, al2_out : al3_in, blood_s5 : exit_s4, die_sum, everything() ) ->
    rel_flows_and_exits
