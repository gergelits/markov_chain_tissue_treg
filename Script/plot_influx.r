...
plot_influx <- function( m )
{
  ...
  fig_filename_start <- get_fig_fname_start( 
    model_id = get_model_id( get_sel_models_file_name( m_tisNA ) ), 
    celltype = m_tisNA$celltype, mcmc_pars = m_tisNA$mcmc_pars )
  
  figs_dir <- get_path( "figs_m", m = m_tisNA, ps = m_tisNA$ps, celltype = m_tisNA$celltype )
