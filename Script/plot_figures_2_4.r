...
plot_figures_2_4 <- function( m_tisNA = NULL, sel_models_file_name, qij_I = 1L )
{
  ...
  # older smaller figure, replaced by more detailed diag_18_params
  if ( FIG_S5B <- FALSE ) {
    gg.fig.S5B <- ggpubr::ggarrange( 
      ...
      ncol = 3, nrow = 3 )
    pdf_name <- sprintf( "%s/%s_Figure_S5B.pdf", figs_dir, fig_filename_start )
    ggexport_my( gg.fig.S5B, filename = pdf_name, 
                 width = 12, height = 8 )
  }
  
  
  if ( FIG_SELECTED <- FALSE ) {
